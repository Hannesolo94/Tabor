// Raise + slightly shrink the print on the tank so it sits in the upper back, not low.
...
// design aspect 1213x1438 (0.8436). Upper-back: ~72% height, near the top.
const h = 1580, w = Math.round(h * 1213 / 1438), top = 120, left = Math.round((1800 - w) / 2);
const position = { area_width: 1800, area_height: 2400, width: w, height: h, top, left, limit_to_print_area: true };
console.log("tank print:", w + "x" + h, "top", top, "left", left);
...
console.log("mockup:", mockUrl ? "ok" : "failed (" + (task.result || JSON.stringify(task)).toString().slice(0, 100) + ")");
if (mockUrl) {
  const mbuf = Buffer.from(await (await fetch(mockUrl)).arrayBuffer()); writeFileSync("/tmp/tank2.jpg", mbuf);
  ...
  const pub = `${SB}/storage/v1/object/public/product-media/the-risen-knight-tank-back.jpg?v=2`;
  ...
  await c.end();
  console.log("updated tank image.");
}
