...
import { isUS } from "@/lib/shipping";
import { countryName } from "@/lib/currency";
import { GOLD, MONO, METAL, CINZEL, BODY } from "@/lib/ui";

export interface ShippingTerms { flat: number; freeOver: number; freeOnlyUS: boolean }

export default function CheckoutForm({ terms, countries, defaultCountry }: {
  terms: ShippingTerms;
  countries: string[];
  ...
  const [form, setForm] = useState({ email: "", name: "", line1: "", line2: "", city: "", province: "", postal: "", country: defaultCountry });
  // Terms come from the server already expressed in the buyer's currency, so the
  // summary can never show a threshold in a different currency to the basket.
  const freeEligible = !terms.freeOnlyUS || isUS(form.country);
  const shippingAmt = lines.length ? (sub >= terms.freeOver && freeEligible ? 0 : terms.flat) : 0;
  const grandTotal = sub + shippingAmt;
  const toFree = freeEligible && shippingAmt > 0 ? terms.freeOver - sub : 0;
              ...
              <div>
                ...
                <div style={{ display: "grid", gap: 10 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    ...
                    <select aria-label="Country" value={form.country} onChange={set("country")} style={inp}>
