...
export default async function Admins() {
  ...
  const inp: React.CSSProperties = { fontFamily: BODY, fontSize: 14, color: "#E8E2D5", background: "rgba(15,15,20,0.6)", border: `1px solid ${GOLD}33`, borderRadius: 10, padding: "11px 13px" };
        ...
        {list.map((a, i) => {
          ...
          return (
            <div key={a.user_id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 16px", borderTop: i ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
              ...
              {canRevoke ? (
                <form action={revokeAdmin}><input type="hidden" name="user_id" value={a.user_id} /><button style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: "0.08em", color: "#C03A3A", background: "rgba(192,58,58,0.06)", border: "1px solid rgba(192,58,58,0.4)", borderRadius: 10, padding: "7px 11px", cursor: "pointer" }}>REVOKE</button></form>
