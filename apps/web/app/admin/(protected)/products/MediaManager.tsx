...
const ctrl: React.CSSProperties = { fontFamily: MONO, fontSize: 8.5, letterSpacing: "0.06em", textTransform: "uppercase", color: "#C3BDB1", background: "none", border: "1px solid rgba(201,169,97,0.3)", padding: "4px 6px", cursor: "pointer" };
  ...
  return (
    <div style={{ marginTop: 30, paddingTop: 22, borderTop: "1px solid rgba(201,169,97,0.14)" }}>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 16 }}>
        <label style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase", color: "#0A0A0A", background: `linear-gradient(180deg,#E8D08C,${GOLD})`, padding: "10px 16px", cursor: "pointer" }}>
      ...
      {media.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 12 }}>
          {media.map((m, i) => (
            <div key={m.id} style={{ border: "1px solid rgba(201,169,97,0.2)", background: "#0E0E12", opacity: m.visible ? 1 : 0.45 }}>
              <div style={{ position: "relative", aspectRatio: "1 / 1", background: "#15151A", overflow: "hidden" }}>
                ...
                {i === 0 && <span style={{ position: "absolute", top: 6, left: 6, fontFamily: MONO, fontSize: 7, color: "#0A0A0A", background: GOLD, padding: "2px 5px", letterSpacing: "0.08em" }}>MAIN</span>}
                {m.type === "video" && <span style={{ position: "absolute", top: 6, right: 6, color: GOLD, fontSize: 14 }}>▶</span>}
                {!m.visible && <span style={{ position: "absolute", bottom: 6, left: 6, fontFamily: MONO, fontSize: 7, color: "#C03A3A", border: "1px solid #C03A3A", padding: "1px 4px" }}>HIDDEN</span>}
              </div>
