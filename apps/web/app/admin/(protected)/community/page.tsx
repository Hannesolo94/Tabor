...
export default async function CommunityPage() {
  ...
  return (
    <div>
      ...
      <div style={{ border: `1px solid ${emailReady ? "rgba(123,191,123,0.4)" : GOLD + "44"}`, background: "#0E0E12", padding: "14px 18px", marginBottom: 22, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10, maxWidth: 560 }}>
        <div>
        ...
        <form action={sendTestEmail}><button type="submit" style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: GOLD, border: `1px solid ${GOLD}55`, padding: "10px 14px", cursor: "pointer", background: "none" }}>Send test email to me</button></form>
      </div>
      ...
      <form action={sendBroadcast} style={{ display: "grid", gap: 12, maxWidth: 560, marginBottom: 30 }}>
        ...
        <div><button type="submit" style={{ fontFamily: MONO, fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "#0A0A0A", background: `linear-gradient(180deg,#E8D08C,${GOLD})`, border: "none", padding: "12px 22px", cursor: "pointer" }}>Send to all members</button></div>
      </form>
