// Content Studio > Broadcast: push an announcement to every member's in-app inbox
// (optionally email too). Lives as its own tab in the Studio; the engine stays in
// the community actions.
import { supabaseServer } from "@/lib/supabase/server";
import { sendBroadcast, sendTestEmail } from "../../community/actions";
...
export default async function StudioBroadcast() {
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontFamily: BODY, fontSize: 13, color: "#9A948A", margin: "6px 0 0", maxWidth: 640 }}>Broadcasts are System-voice announcements: they land in every member's in-app inbox ({users ?? 0} {users === 1 ? "member" : "members"}), with optional email.</p>
        <StudioTabs active="broadcast" />
      </div>
