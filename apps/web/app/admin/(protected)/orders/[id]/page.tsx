...
export default async function OrderDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      ...
      {/* status */}
      <div style={{ marginTop: 22, border: "1px solid rgba(201,169,97,0.16)", background: "#0E0E12", padding: "16px 20px" }}>
        ...
        <form action={updateOrderStatus} style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          {STATUSES.map((s) => (
            <button key={s} name="status" value={s} type="submit" style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: "0.06em", textTransform: "uppercase", padding: "8px 12px", border: `1px solid ${GOLD}44`, cursor: "pointer", color: o.status === s ? "#0A0A0A" : "#9A948A", background: o.status === s ? `linear-gradient(180deg,#E8D08C,${GOLD})` : "transparent" }}>{s}</button>
          ))}
        </form>
      </div>
    </div>
