...
export function DeleteCustomer({ email }: { email: string }) {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: CRIMSON, background: "none", border: `1px solid ${CRIMSON}66`, padding: "10px 16px", cursor: "pointer" }}>
        Delete all data
      </button>
      ...
      {open && (
        <div onClick={() => setOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 400, background: "rgba(0,0,0,0.78)", display: "grid", placeItems: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "min(440px, 95vw)", background: "#0E0E12", border: `1px solid ${CRIMSON}55`, padding: "28px 26px" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 22, justifyContent: "flex-end" }}>
              <button onClick={() => setOpen(false)} style={{ fontFamily: MONO, fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", color: "#C3BDB1", background: "none", border: "1px solid rgba(201,169,97,0.3)", padding: "11px 18px", cursor: "pointer" }}>Cancel</button>
              <form action={deleteCustomer}>
                <input type="hidden" name="email" value={email} />
                <button type="submit" style={{ fontFamily: MONO, fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", color: "#fff", background: CRIMSON, border: "none", padding: "11px 18px", cursor: "pointer" }}>Yes, erase permanently</button>
              </form>
            </div>
          </div>
        </div>
