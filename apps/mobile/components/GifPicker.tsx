    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: C.black, paddingTop: 50 }}>
        <View style={{ flexDirection: "row", gap: 8, paddingHorizontal: 12, paddingBottom: 8, alignItems: "center" }}>
          <TextInput value={q} onChangeText={setQ} onSubmitEditing={() => run(q)} placeholder="Search GIFs…" placeholderTextColor={C.muted} autoFocus style={{ flex: 1, backgroundColor: C.surface, borderWidth: 1, borderColor: C.line, color: C.ivory, paddingHorizontal: 12, paddingVertical: 10, borderRadius: 2, fontFamily: F.body }} />
          <Pressable onPress={() => run(q)} style={{ backgroundColor: C.gold, paddingHorizontal: 14, justifyContent: "center", borderRadius: 2, paddingVertical: 10 }}><Text style={{ color: C.black, fontFamily: F.head }}>GO</Text></Pressable>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable onPress={() => { onPick(item.url); onClose(); }} style={{ flex: 1, margin: 4 }}>
                <Image source={{ uri: item.preview }} style={{ width: "100%", height: 120, borderRadius: 4, backgroundColor: C.surface }} resizeMode="cover" />
              </Pressable>
            )}
          />
        )}
