...
import { traditionOf, orthodoxCalendarOf, type Tradition } from "./disciplines";
...
export async function syncLiturgicalReminders(enabled: boolean, trad: Tradition, old = false, oriental = false): Promise<void> {
  try {
    await cancelLiturgicalReminders();
    if (!enabled) return;
    if (oriental) return; // Oriental Orthodox calendar is its own track; no Eastern dates
    const { status } = await Notifications.getPermissionsAsync();
    if (status !== "granted") return; // permission is requested by registerForPush, not here
    for (const e of upcoming(new Date(), trad, 8, old)) {
...
export async function refreshLiturgicalReminders(userId: string): Promise<void> {
  try {
    const { data } = await supabase.from("profiles").select("notif_prefs, denomination, orthodox_calendar").eq("user_id", userId).maybeSingle();
    const enabled = ((data?.notif_prefs as { push?: { feasts?: boolean } })?.push?.feasts) ?? true;
    const cal = orthodoxCalendarOf(data?.denomination ?? null, (data as { orthodox_calendar?: string | null })?.orthodox_calendar);
    await syncLiturgicalReminders(enabled, traditionOf(data?.denomination ?? null), cal === "old", cal === "oriental");
  } catch { /* notifications optional */ }
}
